...
- Cloudflare is Pivot's approved web host and currently serves `pivotteamwear.com` until the user explicitly approves a change. This approval does not extend to additional Cloudflare products or services. On 7 August 2026 the Director approved local implementation only of a Cloudflare Pages Function and edge-rate-limit design for club-interest delivery through Fastmail JMAP; this does not approve preview or production deployment, provider configuration, secret creation or live-site changes. Do not allocate or migrate to another web host because a requirement is unsupported or the incumbent platform is not yet understood; first establish its capabilities and present any constraint for decision.
