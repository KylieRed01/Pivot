...
- Cloudflare is Pivot's approved web host and currently serves `pivotteamwear.com` until the Director approves a change. This does not approve additional Cloudflare products or services. Confirm account ownership, access and required hosting capabilities before release; do not silently allocate or migrate to another host if a requirement is unsupported. The repository currently contains no Cloudflare deployment configuration, and the public response alone does not establish that the current static deployment can run the Node SMTP endpoints or provide a trusted client-address boundary; verify those capabilities in the approved account before release.
