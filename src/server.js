...
export function createApp(){return createServer(async(req,res)=>{try{
  ...
  const path=url.pathname==='/'?'/index.html':url.pathname.endsWith('/')?url.pathname+'index.html':url.pathname;try{const file=await readFile(join(root,'public',path));res.writeHead(200,{'content-type':types[extname(path)]||'application/octet-stream'});res.end(file);}catch{send(res,404,{error:'Not found'});}
