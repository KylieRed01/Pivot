...
export function renderJerseyPreview(d, design, selectedId, garment = 'basketball-jersey') {
  ...
  const layers = design.layers.map(layer => {
    ...
    const textBoxLimit = layer.type !== 'text' ? 100 : layer.alignment === 'left' ? 100 - layer.x : layer.alignment === 'right' ? layer.x : 2 * Math.min(layer.x, 100 - layer.x);
    const element = layer.type === 'text' ? 'div' : 'button';
    const interaction = layer.type === 'text' ? ' role="button" tabindex="0"' : ' type="button"';
    return `<${element} class="art-layer ${layer.role==='number'?'number-layer':''} ${layer.type==='image'?'image-layer':''} ${layer.libraryAssetId==='pivot-logo'?'pivot-logo-layer':''} ${selectedId===layer.id?'selected':''}" data-layer-id="${layer.id}"${layer.type==='text'?' data-inline-text="true"':''}${interaction} style="--x:${layer.x}%;--anchor-x:${anchorX};--y:${layer.y}%;--layer-scale:${layerScale};--text-box-limit:${textBoxLimit}%;--rotation:${layer.rotation}deg;--layer-colour:${layer.colour||'#FFFFFF'};--crop-zoom:${layer.cropZoom||1};--crop-x:${layer.cropX??50}%;--crop-y:${layer.cropY??50}%;--layer-opacity:${layer.opacity??1};--flip-x:${layer.flipX?-1:1};--flip-y:${layer.flipY?-1:1};--letter-spacing:${layer.letterSpacing??0}px;--line-height:${layer.lineSpacing??1};--text-align:${layer.alignment||'center'}${esc(typography)}" aria-label="${esc(layer.type==='text'?`${layer.role==='number'?'Basketball number':'Text'} ${layer.text}. Press Enter to edit on the garment`:`${layer.name} layer`)}">${layer.type==='image'?`<span class="image-crop-frame"><img src="${layer.src}" alt=""></span>`:`<span class="art-layer-text">${esc(layer.text)}</span>`}${selectedId===layer.id?'<i class="resize-handle" aria-hidden="true">↘</i>':''}</${element}>`;
  }).join('');
